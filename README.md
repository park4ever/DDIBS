...
### 2. 운영 품질 / 정책 보강
- 홀드 만료 경계시각 정책 정리 (`expiresAt == now` 포함)
- `Clock` 주입 기반 시간 정책 테스트 가능 구조
- 배치 오케스트레이터 / ItemProcessor 분리
- 배치 건별 `REQUIRES_NEW` 적용
- 락 획득 순서 점검 및 정책 정리
- 운영 예외 추적 로그 보강
...
- 홀드 만료 배치 재실행 안정성 테스트
- 정산 생성 배치 재실행 안정성 테스트
- 시연 스크립트 문서
- 시퀀스 다이어그램 문서
- 버전별 계획 / 최종 리뷰 문서
...
### 4. 홀드 만료 시각은 경계값까지 정책적으로 관리한다
DDIBS는 홀드 만료 시각을 단순 비교가 아니라 **정책적으로 관리**합니다.

- 홀드 만료 기준은 `currentTime >= expiresAt`
- `Clock` 주입으로 배치 시간 기준을 통일
- `now < expiresAt`, `now == expiresAt`, `now > expiresAt` 경계 테스트 보강

이를 통해 시간 정책이 도메인 / 배치 / 테스트에서 일관되게 동작하도록 정리했습니다.

### 5. 배치는 오케스트레이터와 건별 처리로 분리한다
홀드 만료 배치와 정산 생성 배치는 모두 다음 구조를 따릅니다.

- `BatchService`: 후보 조회 + 결과 집계
- `ItemProcessor`: 실제 한 건 처리
- 건별 처리에는 `REQUIRES_NEW` 적용

이를 통해 배치 전체 단일 트랜잭션 구조를 줄이고, 부분 실패 후 재실행 설명력을 높였습니다.

### 6. 락 획득 순서를 실제 코드 기준으로 관리한다
DDIBS는 락 순서를 추상적으로 말하지 않고, 실제 write 경로 기준으로 점검합니다.

- 기존 주문 변경 기반 재고 관련 경로는 `Order → HoldReservation → LaunchVariant` 순서를 따름
- 주문 생성은 `LaunchVariant` 선점 후 `Order`, `HoldReservation` 생성하는 예외 경로로 관리

이를 통해 과도한 구조 변경 없이도 락 정책을 설명 가능하게 유지합니다.

### 7. 배치는 결과를 구조적으로 반환한다
...
- `SettlementBatchService`
  ...
  - 중복 정산 생성 방지
  - 미확정 주문 제외
  - 부분 실패 후 재실행 시 남은 건만 다시 처리되는지 검증
...
- `docs/DDIBS_V5_PLAN.md`
- `docs/DDIBS_V5_FINAL_REVIEW.md`
...
까지 수행해 시간 정책을 코드와 테스트에서 일관되게 설명할 수 있도록 정리했습니다.

### 4. 배치 오케스트레이터와 ItemProcessor를 분리한 이유
배치 전체를 하나의 큰 트랜잭션으로 처리하지 않고,

- 배치 서비스는 후보 조회와 결과 집계
- ItemProcessor는 한 건 처리
- 건별 `REQUIRES_NEW`

구조로 분리해 부분 실패 후 재실행 가능한 구조를 만들었습니다.

### 5. 동시성 테스트로 과판매 방지 확인
재고 1개인 상황에서 동시 주문 테스트를 작성해,  
1건만 성공하고 나머지는 실패하는지 확인했습니다.

### 6. 결제 vs 홀드 만료 배치 경합 처리
...
에 대한 서버 로그를 `GlobalExceptionHandler` 중심으로 보강했습니다.
